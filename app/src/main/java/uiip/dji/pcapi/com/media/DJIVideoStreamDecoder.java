package uiip.dji.pcapi.com.media;
...
public class DJIVideoStreamDecoder implements NativeHelper.NativeDataListener {
    ...
    private void initCodec() {
        ...
        Logger.log("initVideoDecoder video width = " + width + "  height = " + height);
    ...
    private void decodeFrame() throws Exception {
        ...
        int i = 0;
        for (i = 0; i < CODEC_DEQUEUE_INPUT_QUEUE_RETRY && inIndex < 0; i ++) {
            try {
                inIndex = codec.dequeueInputBuffer(1000);
            } catch (IllegalStateException e) {
                logd(TAG, "decodeFrame: dequeue input: " + e);
                Logger.log("decodeFrame: dequeue input: " + e);
                codec.stop();
                codec.reset();
                initCodec();
                e.printStackTrace();
        ...
        //Logger.log("decodeFrame: index=" + inIndex +"times"+i);
        ...
        if (inIndex >= 0) {
            ...
            int outIndex = -1;
            outIndex = codec.dequeueOutputBuffer(bufferInfo, 1000);
            logd(TAG, "decodeFrame: outIndex: " + outIndex);
            //Logger.log("decodeFrame: outIndex: " + outIndex);
            if (outIndex >= 0) {
    ...
    public void onDataRecv(byte[] data, int size, int frameNum, boolean isKeyFrame, int width, int height) {
        ...
        if (data.length != size) {
            ...
            //Logger.log("recv data size: " + size + ", data lenght: " + data.length);
        } else {
            logd( "recv data size: " + size + ", frameNum: "+frameNum+", isKeyframe: "+isKeyFrame+"," +
                    " width: "+width+", height: " + height);
            //Logger.log("recv data size: " + size + ", frameNum: "+frameNum+", isKeyframe: "+isKeyFrame+"," +
             //       " width: "+width+", height: " + height);
